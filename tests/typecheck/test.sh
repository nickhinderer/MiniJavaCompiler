...
for file in *; do
  ...
  if [[ $(grep 'oracle' $file) = *pass* ]]; then
    if [[ $(diff ../oracle/pass_output <(java -cp "/home/nick/scpdir/Typecheck/target/classes" Typecheck <"$file")) = *---* ]]; then
      echo -e '\e[0;31mFAIL\e[0;30m'
    else
      echo -e '\e[0;32mPASS\e[0;30m'
    fi
  else
    if [[ $(diff ../oracle/fail_output <(java -cp "/home/nick/scpdir/Typecheck/target/classes" Typecheck <"$file")) = *---* ]]; then
      echo -e '\e[0;31mFAIL\e[0;30m'
    else
      echo -e '\e[0;32mPASS\e[0;30m'
    fi
